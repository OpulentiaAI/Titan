    ...
    switch (action) {
      ...
      case 'fill':
        if (value) {
          ...
          console.log(`🔧 [fill] Starting fill action: selector="${target}", text="${textToType.substring(0, 30)}..."`);
          ...
          if (target && !target.includes(':focus')) {
            ...
            console.log(`🔍 [fill] Queried selector "${target}": ${element ? '✓ found' : '✗ not found'}`);
          }
          ...
          if (!element) {
            ...
            console.log(`🔍 [fill] Using focused element: ${element?.tagName || 'none'}`);
          }

          if (element && (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA' ||
                         element.getAttribute('contenteditable') === 'true')) {

            console.log(`✓ [fill] Element is typeable: ${element.tagName}`);
            
            // Click the element first to ensure it receives focus
            const rect = element.getBoundingClientRect();
            const clickX = rect.left + rect.width / 2;
            const clickY = rect.top + rect.height / 2;
            console.log(`🖱️ [fill] Clicking element at (${clickX}, ${clickY})`);
            ...
            return new Promise<any>((resolve) => {
              console.log(`⏱️ [fill] Starting 300ms focus delay`);
              
              setTimeout(() => {
                console.log(`✓ [fill] Focus delay complete, verifying focus...`);
                
                // Verify element still has focus
                const stillFocused = document.activeElement === element;
                console.log(`🔍 [fill] Still focused: ${stillFocused ? '✓ yes' : '✗ no'}`);

                if (!stillFocused) {
                  console.log(`🔄 [fill] Re-focusing element...`);
                  element!.focus();
                  // Add another small delay after re-focus
                  setTimeout(() => {
                    console.log(`⌨️ [fill] Proceeding with typing after re-focus...`);
                    proceedWithTyping();
                  }, 100);
                } else {
                  console.log(`⌨️ [fill] Proceeding with typing...`);
                  proceedWithTyping();
                }

                function proceedWithTyping() {
                  console.log(`📝 [fill] proceedWithTyping called`);
                  ...
                  if (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') {
                    ...
                    // Use call() to ensure proper 'this' context
                    const dispatchEventBound = inputElement.dispatchEvent.bind(inputElement);
                    dispatchEventBound(new Event('input', { bubbles: true }));
                    dispatchEventBound(new Event('change', { bubbles: true }));
                    dispatchEventBound(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
                    dispatchEventBound(new KeyboardEvent('keypress', { key: 'Enter', bubbles: true }));
                    dispatchEventBound(new KeyboardEvent('keyup', { key: 'Enter', bubbles: true }));


                  } else if (element.getAttribute('contenteditable') === 'true') {
                    // For contenteditable elements, clear and set text
                    element.textContent = textToType;
                    const dispatchEventBound = element.dispatchEvent.bind(element);
                    dispatchEventBound(new Event('input', { bubbles: true }));
                    dispatchEventBound(new Event('change', { bubbles: true }));
                  }

                  console.log(`✅ [fill] Successfully typed "${textToType.substring(0, 30)}..." into ${element.tagName}`);
                  resolve({
                    success: true,
          ...
          console.error('❌ [fill] Element not typeable:', element?.tagName, element);
          return {
            success: false,
...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
  ...
  if (request.type === 'EXECUTE_ACTION') {
    // Handle asynchronously to properly support Promise-returning actions
    (async () => {
      try {
        ...
        // Properly await Promise results
